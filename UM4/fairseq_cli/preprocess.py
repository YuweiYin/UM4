...
def main(args):
    ...
    def make_binary_dataset(vocab, input_prefix, output_prefix, lang, num_workers):
        ...
        merge_result(
            Binarizer.binarize(
                input_file, vocab, lambda _t: ds.add_item(_t), offset=0, end=offsets[1]
            )
        )
        if num_workers > 1:
            pool.join()
    ...
    def make_all(lang, vocab):
        ...
        if args.validpref:
            for _k, validpref in enumerate(args.validpref.split(",")):
                outprefix = "valid{}".format(_k) if _k > 0 else "valid"
                make_dataset(
                    vocab, validpref, outprefix, lang, num_workers=args.workers
                )
        if args.testpref:
            for _k, testpref in enumerate(args.testpref.split(",")):
                outprefix = "test{}".format(_k) if _k > 0 else "test"
