...
def main(cfg: DictConfig, **unused_kwargs):
    ...
    # Optimize ensemble for generation and set the source and test dicts on the model (required by scorer)
    for model in models:
        if use_fp16:
            model.half()
    ...
    for sample in progress:
        ...
        for i, hypos_i in enumerate(hypos):
            ...
            if bpe_toks is not None:
                for _i in range(tgt_len - 1):
                    if tokens[_i].item() in bpe_toks:
                        skipped_toks += 1
                        pos_scores[_i + 1] += pos_scores[_i]
                        pos_scores[_i] = 0
            ...
            if cfg.eval_lm.output_word_probs or cfg.eval_lm.output_word_stats:
                ...
                for _i in range(len(tokens)):
                    w_ind = tokens[_i].item()
                    w += task.source_dictionary[w_ind]
                    if bpe_toks is not None and w_ind in bpe_toks:
                        w = w[:-bpe_len]
                        is_bpe = True
                    else:
                        word_prob.append((w, pos_scores[_i].item()))
