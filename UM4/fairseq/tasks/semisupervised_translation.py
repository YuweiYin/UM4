...
class SemisupervisedTranslationTask(MultilingualTranslationTask):
    ...
    def load_dataset(self, split, epoch=1, **kwargs):
        ...
        def split_exists(_split, _src, _tgt, lang):
            if _src is not None:
                _filename = os.path.join(
                    data_path, "{}.{}-{}.{}".format(_split, _src, _tgt, lang)
                )
            else:
                _filename = os.path.join(
                    data_path, "{}.{}-None.{}".format(_split, _src, _tgt)
                )
            return indexed_dataset.dataset_exists(_filename, impl=self.args.dataset_impl)
        ...
        def language_pair_dataset(_lang_pair):
            _src, _tgt = _lang_pair.split("-")
            src_dataset, tgt_dataset = src_datasets[_lang_pair], tgt_datasets[_lang_pair]
