...
def compute_mask_indices(
    ...
    for i in range(bsz):
        ...
        if no_overlap:
            ...
            def arrange(s, e, _length, keep_length):
                span_start = np.random.randint(s, e - _length)
                mask_idc.extend(span_start + _i for _i in range(_length))

                new_parts = []
                if span_start - s - min_space >= keep_length:
                    new_parts.append((s, span_start - min_space + 1))
                if e - span_start - keep_length - min_space > keep_length:
                    new_parts.append((span_start + _length + min_space, e))
                return new_parts
