...
def collate(
    ...
    if pad_to_length is not None and "prepend_target" in pad_to_length.keys():
        batch["net_input"]["prepend_target"] = prepend_target
    if prev_output_tokens is not None:
        batch["net_input"]["prev_output_tokens"] = prev_output_tokens.index_select(
            0, sort_order
        )
    if samples[0].get("score", None) is not None:
        batch["score"] = torch.cat([sample["score"] for sample in samples], dim=0).index_select(0, sort_order)
...
class LanguagePairDataset(FairseqDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.score_dataset is not None:
            example["score"] = self.score_dataset[index] if self.score_dataset is not None else None
