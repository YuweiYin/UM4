...
class MultilingualDatasetManager(object):
    def __init__(self, args, lang_pairs, langs, dicts, sampling_method, truncate_source=False, truncate_target=False):
        ...
        if args.score_temperature is None:
            self.score_temperature = 1 / math.sqrt(1024)
        else:
            self.score_temperature = args.score_temperature
        if args.score_size is None:
            self.score_size = 4
        else:
            self.score_size = args.score_size
        self.seed = args.seed
    ...
    def add_args(parser):
        ...
        parser.add_argument('--score-temperature', type=float,
                            help='to balance softmax')
        parser.add_argument('--score-size', type=int,
                            help='to balance softmax')
    ...
    @classmethod
    def prepare(cls, load_dictionary, args, **kwargs):
        ...
        if args.source_lang is not None or args.target_lang is not None:
            ...
            training = True
        language_list = cls.load_langs(args, **kwargs)
        check_langs(
            language_list,
            (
    ...
    def load_lang_dataset(
        ...
        score_data_set = None
        score_path = os.path.join(data_path, "{}.{}-{}.score".format(split, src, tgt))
        src_score_path = os.path.join(data_path, "{}.{}-{}.src-score".format(split, src, tgt))
        tgt_score_path = os.path.join(data_path, "{}.{}-{}.tgt-score".format(split, src, tgt))
        if os.path.exists(score_path) and split == "train" and self.score_size > 0:
            with open(score_path, "r", encoding="utf-8") as r:
                logger.info("Start reading the score dataset: {}".format(score_path))
                score_data_set = [float(line.strip()) for line in r]
                assert len(score_data_set) % self.score_size == 0, "{}: {} lines !".format(score_path,
                                                                                           len(self.score_size))
                score_data_set = torch.FloatTensor(score_data_set).view(len(score_data_set) // self.score_size,
                                                                        self.score_size)
                score_data_set = torch.exp(score_data_set)
                score_data_set = utils.softmax(score_data_set / self.score_temperature, dim=1).view(-1).unsqueeze(1)
                score_data_set = score_data_set * self.score_size
                logger.info("Complete preprocessing the score dataset: {}".format(score_path))
        elif os.path.exists(src_score_path) and os.path.exists(
                tgt_score_path) and split == "train" and self.score_size > 0:
            with open(src_score_path, "r", encoding="utf-8") as src_r:
                with open(tgt_score_path, "r", encoding="utf-8") as tgt_r:
                    logger.info("Start reading the score dataset: {} | {}".format(src_score_path, tgt_score_path))
                    src_score_data_set = [float(line.strip()) for line in src_r]
                    assert len(src_score_data_set) % self.score_size == 0
                    src_score_data_set = torch.FloatTensor(src_score_data_set).view(
                        len(src_score_data_set) // self.score_size, self.score_size)
                    src_score_data_set = torch.exp(src_score_data_set)

                    tgt_score_data_set = [float(line.strip()) for line in tgt_r]
                    assert len(tgt_score_data_set) % self.score_size == 0
                    tgt_score_data_set = torch.FloatTensor(tgt_score_data_set).view(
                        len(tgt_score_data_set) // self.score_size, self.score_size)
                    tgt_score_data_set = torch.exp(tgt_score_data_set)

                    score_data_set = src_score_data_set * tgt_score_data_set
                    score_data_set = utils.softmax(score_data_set / self.score_temperature, dim=1).view(-1).unsqueeze(1)
                    score_data_set = score_data_set * self.score_size
                    logger.info(
                        "Complete preprocessing the score dataset: {} | {}".format(src_score_path, tgt_score_path))
        ...
        return src_dataset, tgt_dataset, align_dataset, score_data_set
    ...
    def load_langpair_dataset(
        ...
        # a hack: anyone is not in cache, we need to reload them
        if (
            ...
            src_dataset, tgt_dataset, align_dataset, score_dataset = self.load_lang_dataset(
                data_path,
                split,
                src,
                src_dict,
    ...
    def get_train_dataset_sizes(
        ...
        #     # use the max known data size to approximate.
    ...
    def load_sampled_multi_epoch_dataset(
        ...
        if training and split == getattr(self.args, "train_subset", None):
            ...
            return SampledMultiEpochDataset(
                ...
                # valid and test datasets will be degenerate to concatenating datasets:
