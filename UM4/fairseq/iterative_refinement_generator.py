...
class IterativeRefinementGenerator(object):
    ...
    def generate(self, models, sample, prefix_tokens=None, constraints=None):
        ...
        def finalized_hypos(_step, prev_out_token, prev_out_score, prev_out_attn):
    ...
    def rerank(self, reranker, finalized, encoder_input, beam_size):
        def rebuild_batch(_finalized):
            finalized_tokens = [f[0]["tokens"] for f in _finalized]
            finalized_maxlen = max(f.size(0) for f in finalized_tokens)
            _final_output_tokens = (
                finalized_tokens[0]
                .new_zeros(len(finalized_tokens), finalized_maxlen)
                .fill_(self.pad)
            )
            for _i, f in enumerate(finalized_tokens):
                _final_output_tokens[_i, : f.size(0)] = f
            return _final_output_tokens
