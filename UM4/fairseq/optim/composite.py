...
class FairseqCompositeOptimizer(FairseqOptimizer):
    ...
    def __init__(self, cfg: CompositeOptimizerConfig, params):
        ...
        for p in params:
            ...
            grouped_params[group].append(p)

        assert grouped_params.keys() == cfg.groups.keys(), (
            f"Parameter groups {grouped_params.keys()} and optimizer groups {cfg.groups.keys()} are not the same! "
            "Try setting 'param_group' on your parameters in the model."
        )

        for group, group_params in grouped_params.items():
