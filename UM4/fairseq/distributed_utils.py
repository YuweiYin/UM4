...
def infer_init_method(cfg: DistributedTrainingConfig, force_distributed=False):
    ...
    if all(
        ...
        key in os.environ
        for key in [
            "PHILLY_RUNTIME_CONFIG", "OMPI_COMM_WORLD_SIZE", "OMPI_COMM_WORLD_RANK", "OMPI_COMM_WORLD_LOCAL_RANK"]
    ):
        ...
        if node_list is not None:
            try:
                ...
                if ntasks_per_node == 1:
                    ...
                    # the first node, [1, 2] on the second node, etc.,
                    # This matches torch.distributed.launch.
                    node_id = int(os.environ.get("SLURM_NODEID"))
...
def all_reduce_dict(data: Mapping[str, Any], device, group) -> Dict[str, Any]:
    ...
    def _all_reduce_dict(data: OrderedDict):
        if len(data) == 0:
            return data
        buf = torch.cat([_t.view(-1) for _t in data.values()]).to(device=device)
        all_reduce(buf, group=group)
        split_buf = torch.split(buf, [_t.numel() for _t in data.values()])
        reduced_data = [_t.view_as(orig) for _t, orig in zip(split_buf, data.values())]
