    ...
    for key in infoxlm_state_dict.keys():
        ...
        if 'lm_head' not in key:
            if 'in_proj_weight' in key:
                ...
                if i_layer % 2 == 0:
                    state_dict[key.replace('decoder.sentence_encoder.', '').replace(
                        'in_proj_bias', 'q_proj.bias').replace(f'.{i_layer}', f'.{i_layer // 2}')] = q
                    state_dict[key.replace('decoder.sentence_encoder.', '').replace(
                        'in_proj_bias', 'k_proj.bias').replace(f'.{i_layer}', f'.{i_layer // 2}')] = k
                    state_dict[key.replace('decoder.sentence_encoder.', '').replace(
                        'in_proj_bias', 'v_proj.bias').replace(f'.{i_layer}', f'.{i_layer // 2}')] = v
                else:
                    state_dict[key.replace('decoder.sentence_encoder.', '').replace(
    ...
    for key in infoxlm_state_dict.keys():
        ...
        if 'lm_head' not in key:
            if 'in_proj_weight' in key:
                ...
                if i_layer % 2 == 0:
                    state_dict[key.replace('decoder.sentence_encoder.', '').replace(
                        'in_proj_weight', 'q_proj.weight').replace(f'.{i_layer}', f'.{i_layer // 2}')] = q
                    state_dict[key.replace('decoder.sentence_encoder.', '').replace(
                        'in_proj_weight', 'k_proj.weight').replace(f'.{i_layer}', f'.{i_layer // 2}')] = k
                    state_dict[key.replace('decoder.sentence_encoder.', '').replace(
                        'in_proj_weight', 'v_proj.weight').replace(f'.{i_layer}', f'.{i_layer // 2}')] = v
                else:
                    state_dict[key.replace('decoder.sentence_encoder.', '').replace(
                        'in_proj_weight', 'q_proj.weight').replace(f'.{i_layer}', f'.{i_layer // 2}').replace(
                        'self_attn', 'encoder_attn')] = q
                    state_dict[key.replace('decoder.sentence_encoder.', '').replace(
                        'in_proj_weight', 'k_proj.weight').replace(f'.{i_layer}', f'.{i_layer // 2}').replace(
                        'self_attn', 'encoder_attn')] = k
                    state_dict[key.replace('decoder.sentence_encoder.', '').replace(
                        'in_proj_weight', 'v_proj.weight').replace(f'.{i_layer}', f'.{i_layer // 2}').replace(
                ...
                if i_layer % 2 == 0:
                    ...
                    state_dict[key.replace('decoder.sentence_encoder.', '').replace(
                        'in_proj_bias', 'q_proj.bias').replace(f'.{i_layer}', f'.{i_layer // 2}').replace(
                        'self_attn', 'encoder_attn')] = q
                    state_dict[key.replace('decoder.sentence_encoder.', '').replace(
                        'in_proj_bias', 'k_proj.bias').replace(f'.{i_layer}', f'.{i_layer // 2}').replace(
                        'self_attn', 'encoder_attn')] = k
                    state_dict[key.replace('decoder.sentence_encoder.', '').replace(
                        'in_proj_bias', 'v_proj.bias').replace(f'.{i_layer}', f'.{i_layer // 2}').replace(
                        'self_attn', 'encoder_attn')] = v
            elif 'fc1' in key:
                i_layer = int(key.split('.')[3])
                if i_layer % 2 == 0:
                ...
                if i_layer % 2 == 0:
                    state_dict[key.replace('decoder.sentence_encoder.', '').replace(
                        'final_layer_norm', 'ffn_layer_norm').replace(f'.{i_layer}', f'.{i_layer // 2}')] = \
                        infoxlm_state_dict[key]
                else:
                    state_dict[key.replace('decoder.sentence_encoder.', '').replace(
                        f'.{i_layer}', f'.{i_layer // 2}')] = infoxlm_state_dict[key]
...
class XLMTLSDecoder(TransformerDecoder):
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        if self.decoder_ls_depth_strategy == "static-same":
            ...
            self.decoder_ls_layers_list = [int(layer_num) for layer_num in args.decoder_ls_layers_list.split(',')] if \
                hasattr(args, "decoder_ls_layers_list") else [0] * len(self.langs)
            assert len(self.decoder_ls_layers_list) == len(self.langs)
            for i in range(len(self.langs)):
        ...
        self.softmax_temperature = args.softmax_temperature if \
            hasattr(args, "softmax_temperature") else math.sqrt(args.decoder_embed_dim)
        ...
        if self.decoder_group_type == "sparse":
            ...
            exit()
        self.language_specific_lr = [
            float(w) for w in args.language_specific_lr.split(",")] if hasattr(args, "language_specific_lr") else None
    ...
    def extract_features(
        ...
        if self.decoder_ls_depth_strategy == "static-same":
            x = torch.stack([inner_states[i] for i in range(
                self.decoder_group_layers - 1, len(inner_states), self.decoder_group_layers)], dim=0)
            ...
            if self.decoder_group_type == "sparse":
                x = torch.stack([inner_states[i] for i in range(
                    self.decoder_group_layers - 1, len(inner_states), self.decoder_group_layers)])
                loss_weight = self.loss_weight
                loss_weight = utils.softmax(loss_weight / self.softmax_temperature, dim=-1)
            elif self.decoder_group_type == "ls:sparse":
                x = torch.stack([inner_states[i] for i in range(
