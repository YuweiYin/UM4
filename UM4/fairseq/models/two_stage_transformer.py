    ...
    for key in infoxlm_state_dict.keys():
        ...
        if 'lm_head' not in key:
            if "adapter" in key:
                ...
                if i_layer % 2 == 0:
                    state_dict[
                        key.replace('decoder.', '').replace('fc1', 'fc3').replace(
                            f'.{i_layer}', f'.{i_layer // 2}')] = infoxlm_state_dict[key]
                else:
                    state_dict[key.replace('decoder.', '').replace(f'.{i_layer}', f'.{i_layer // 2}')] = \
                        infoxlm_state_dict[key]
            elif 'fc2' in key:
                i_layer = int(key.split('.')[2])
                if i_layer % 2 == 0:
                    state_dict[
                        key.replace('decoder.', '').replace('fc2', 'fc4').replace(
                            f'.{i_layer}', f'.{i_layer // 2}')] = infoxlm_state_dict[key]
                else:
                    state_dict[key.replace('decoder.', '').replace(f'.{i_layer}', f'.{i_layer // 2}')] = \
                        infoxlm_state_dict[key]
                ...
                if i_layer % 2 == 1:
                    state_dict[
                        key.replace('decoder.', '').replace('self_attn.out_proj', 'encoder_attn.out_proj').replace(
                            f'.{i_layer}', f'.{i_layer // 2}')] = infoxlm_state_dict[key]
                else:
                    state_dict[key.replace('decoder.', '').replace(f'.{i_layer}', f'.{i_layer // 2}')] = \
                        infoxlm_state_dict[key]
                ...
                if i_layer % 2 == 1:
                    state_dict[key.replace('decoder.', '').replace(
                        'self_attn', 'encoder_attn').replace(f'.{i_layer}', f'.{i_layer // 2}')] = \
                        infoxlm_state_dict[key]
                else:
                    state_dict[key.replace('decoder.', '').replace(f'.{i_layer}', f'.{i_layer // 2}')] = \
                        infoxlm_state_dict[key]
                ...
                if i_layer % 2 == 1:
                    state_dict[
                        key.replace('decoder.', '').replace('self_attn_layer_norm', 'encoder_attn_layer_norm').replace(
                            f'.{i_layer}', f'.{i_layer // 2}')] = infoxlm_state_dict[key]
                else:
                    state_dict[key.replace('decoder.', '').replace(f'.{i_layer}', f'.{i_layer // 2}')] = \
                        infoxlm_state_dict[key]
                ...
                state_dict[key.replace('decoder.', '')] = infoxlm_state_dict[key][
                                                          :state_dict[key.replace('decoder.', '')].size(0)]
            elif 'embed_tokens' in key:
...
class TwoStageDecoder(TransformerDecoder):
    ...
    def extract_features_scriptable(
        ...
        if self.use_adapter:
            self.swap_adapter_forward(x, tgt_lang_id)
