    ...
    *passthrough_args* will be passed through to
    ...
    if (
        ...
        if cfg.finetune_from_model is not None and first_launch:
            ...
            # else just use usual logic to load checkpoint, e.g. restart from last checkpoint etc.
...
def prune_state_dict(state_dict, model_cfg: Optional[DictConfig]):
    ...
    def create_pruning_pass(layers_to_keep, _layer_name):
        keep_layers = sorted(
        ...
        regex = re.compile(r"^{layer}.*\.layers\.(\d+)".format(layer=_layer_name))
