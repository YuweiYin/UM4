...
class SequenceGenerator(nn.Module):
    ...
    def transpose_list(self, _l: List[List[int]]):
        # GeneratorExp aren't supported in TS so ignoring the lint
        min_len = min([len(x) for x in _l])  # noqa
        l2 = [[row[i] for row in _l] for i in range(min_len)]
        return l2
    ...
    def _no_repeat_ngram(self, tokens, lprobs, bsz: int, beam_size: int, step: int):
        ...
        if step + 2 - self.no_repeat_ngram_size >= 0:
            ...
            banned_tokens = [
                torch.jit.annotate(List[int], []) for _ in range(bsz * beam_size)
            ]
        for bbsz_idx in range(bsz * beam_size):
            lprobs[bbsz_idx][
...
class SequenceGeneratorWithAlignment(SequenceGenerator):
    def __init__(self, models, tgt_dict, left_pad_target=False, **kwargs):
        ...
        Args:
            left_pad_target (bool, optional): Whether the
